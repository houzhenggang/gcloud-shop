...
public class AreaInfoServiceImpl implements IAreaInfoService {
    ...
    public int deleteByPrimaryKey(Long id) throws ServcieException {
        ...
        try {
            ...
            throw new ServcieException(Constant.API_CALL_ERROR, "删除区域信息报错!");
        }
        return result;
    }

    @Override
    public int insert(AreaInfo record) throws ServcieException {
        int result = 0;
        try {
            result = areaInfoMapper.insert(record);
        } catch (Exception e){
            logger.error(e.getMessage());
            throw new ServcieException(Constant.API_CALL_ERROR, "添加区域信息报错!");
        }
        return result;
    }

    @Override
    public int insertSelective(AreaInfo record) throws ServcieException {
        int result = 0;
        try {
    ...
    @Override
    public AreaInfo selectByPrimaryKey(Long id) throws ServcieException {
    ...
    @Override
    public int updateByPrimaryKeySelective(AreaInfo record) throws ServcieException {
        int result = 0;
        try {
            result = areaInfoMapper.updateByPrimaryKeySelective(record);
        } catch (Exception e){
            logger.error(e.getMessage());
            throw new ServcieException(Constant.API_CALL_ERROR, "修改区域信息报错!");
        }
        return result;
    }

    @Override
    public int updateByPrimaryKey(AreaInfo record) throws ServcieException {
        int result = 0;
        try {
